...
3. 結果を確認します。特に以下の項目について確認します。
  ...
  - ログのインジェスト量が上昇していた場合 → [日次上限を設定しデータのインジェスト量を一定量までに抑える](#日次上限を設定しデータのインジェスト量を一定量までに抑える) をご確認ください。
...
以下 2 つは、不要なログの収集を停止する主な例です。
##### ■ 診断設定の収集項目を減らす
マシンの診断設定を有効化している場合、既定の設定では、各種ログについて、複数項目のログが収集されます。  
この設定を見直し、必要な項目についてのみログを収集することが、インジェスト量削減に有効です。  

以下のように、Azure portal の VM の [診断設定] から、収集設定を変更することができます。  
例) Windows VM の パフォーマンス　カウンターの設定  
![](./HowToManageLogAnalyticsBilling/portal-diagnostics-perfc.png)
...
保持するデータ量は、データのインジェスト量とその保有期間によって決まるため、データ保持にかかるコストを抑えるためには、まずは前述の[データのインジェスト量の見直し](#データのインジェストにかかるコストを抑える)と、以下でご案内する、[保有期間の調整](#2-2-1-データの保有期間を調整する)が有効です。
