...
それぞれの確認方法については後述いたします。
- データのインジェスト量の推移
- データの保有量の推移
- データの保有期間

※ 期待せずコストが増加した場合、上記以外にも要因がある場合がございます。一例として、ログの復元を行った場合の復元コストが挙げられます。  
復元のコストはその課金体系上、少量のデータ、短期間の復元であっても予期せず高額になることがあるため、ご注意ください。  
具体的な課金体系およびコストの計算方法については以下弊社公開情報、ブログを参照ください。  
[Azure Monitor でログを復元する | 価格モデル](https://learn.microsoft.com/ja-jp/azure/azure-monitor/logs/restore?tabs=api-1#pricing-model)  
[Log Analytics のクエリの実行コストについて | 3-2. 復元のコスト](https://jpazmon-integ.github.io/blog/LogAnalytics/LogAnalyticsQueryPricing/#3-2-%E5%BE%A9%E5%85%83%E3%81%AE%E3%82%B3%E3%82%B9%E3%83%88)
...
- ストレージ アカウント、または Event Hubs にエクスポート・保存されるデータは、Log Analytics ワークスペースにも収集されます。  
  そのため、コストを抑えたい場合は、テーブルの保有期間を無料期間の範囲に収めるなど、保有期間を短くする必要があります。
