...
### 3-2. Azure Monitor エージェントでログを収集する
#### 3-2-1. Log Analytics ワークスペースに収集する方法
[データ収集ルール (DCR) の作成](https://learn.microsoft.com/ja-jp/azure/azure-monitor/vm/data-collection?tabs=current#create-a-data-collection-rule)により、Azure Monitor エージェントを使用して上記で確認したログを Log Analytics ワークスペースに収集します。
...
#### 3-2-2. ストレージ アカウントに収集する方法
Azure Diagnostics 拡張機能 (WAD/LAD) を使用して収集しているログは基本的にはストレージ アカウントに収集されています。

[Log Analytics ワークスペースにて対象ログを収集しているテーブルに対してデータ エクスポート ルールを作成](https://learn.microsoft.com/ja-jp/azure/azure-monitor/logs/logs-data-export?tabs=portal#create-or-update-a-data-export-rule)することで、Log Analytics ワークスペースに収集されたログをストレージ アカウントに送信することが可能です。


> [!WARNING]
> [Azure Monitor エージェントを使用してログをストレージ アカウントに直接送信する方法](https://learn.microsoft.com/ja-jp/azure/azure-monitor/agents/azure-monitor-agent-send-data-to-event-hubs-and-storage?tabs=windows%2Cwindows-1)は、2026 年 3 月時点ではプレビュー機能として提供されているものの、2026 年 7 月 31 日に廃止予定です。
> Azure Monitor エージェントでストレージ アカウントに直接送信できるログの種類は [公開情報](https://learn.microsoft.com/ja-jp/azure/azure-monitor/agents/azure-monitor-agent-send-data-to-event-hubs-and-storage?tabs=windows%2Cwindows-1#migration-from-azure-diagnostic-extensions-for-linux-and-windows-ladwad) に記載されております。
