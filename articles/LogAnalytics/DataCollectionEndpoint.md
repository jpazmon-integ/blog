...
[カスタム ログ](https://learn.microsoft.com/ja-jp/azure/azure-monitor/agents/data-collection-text-log?tabs=portal)、および [IIS ログ](https://learn.microsoft.com/ja-jp/azure/azure-monitor/agents/data-collection-iis)を取り込む際または[ログ インジェスト API ](https://learn.microsoft.com/ja-jp/azure/azure-monitor/logs/logs-ingestion-api-overview)を使用したログ収集を行う際に DCE が必要となる場合があります。

>[!NOTE] ログ インジェスト API (新規作成分) 、カスタム ログ、IIS ログにおいて、DCE は必須ではなくなりました。 (既存の環境に変更を加える必要はありません。)
>ただし、ファイアウォールなどで宛先の通信制限をする場合は引き続き、DCE を設定いただき当該エンドポイントの通信を許可ください。

ログ インジェスト API の DCE については以下公開情報をご参照ください。
...
Azure Monitor が収集したメトリック データが収集されます。2026 年 1 月時点では、Prometheus に関連するメトリックのみが収集対象です。
</details>
...
#### 目的
ログ インジェスト エンドポイントを設定します。

#### 制限事項
* 送信先の Log Analytics ワークスペース、およびデータ収集ルールと同じリージョンに存在する必要があります。

#### 補足 1 : ログ インジェスト エンドポイントの設定について
ログ インジェスト API (新規作成分) 、カスタム ログ、IIS ログにおいて設定が必須ではなくなりました。 (既存の環境に変更を加える必要はありません。)
この場合、ログは Log Analytics ワークスペース リソース固有のエンドポイントを介して収集されます。
ファイアウォールなどで宛先の通信制限をする場合は引き続き、DCE を設定いただき当該エンドポイントの通信を許可ください。
...
>[!NOTE] 2026 年 1 月時点で、Prometheus メトリックを Azure Monitor ワークスペースに収集するデータ収集ルールを作成に伴い、DCE は新規作成されないよう仕様変更となっております。
>代わりに、 Azure Monitor ワークスペース作成時に自動的に作成されるデータ収集エンドポイントが利用されます。
...
### ログ インジェスト API を使用したい / カスタム ログ または IIS ログを収集したい (通信制御構成がある場合)
* ファイアウォールなどで宛先の通信制限されている場合に、ログ インジェスト エンドポイント用の DCE が必要です。
...
### ログ インジェスト API を使用したい / カスタム ログ または IIS ログを収集したい (通信制御構成がある場合) かつ Azure Private Link Scope を使用してログを収集したい
...
なお、ログ インジェスト エンドポイントと構成アクセス エンドポイントの両方が必要な場合、それぞれのエンドポイントの条件を満たしていれば、 1 つのエンドポイントをログ インジェスト エンドポイントと構成アクセス エンドポイントの両方に使用することが可能です。
...
そして必要なデータ収集エンドポイントの数は Log Analytics ワークスペースおよび VM が存在するリージョンによって変化します。