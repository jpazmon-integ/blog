...
※ 詳細は後述の [<参考サイト>](#lt-参考サイト-gt) もご参照ください。
...
現在は新しいアラート ルールにおいて、通知メールの件名をカスタマイズできる機能が
プレビューとして提供されています。
...
この機能を利用することで、ログ検索アラートの電子メールの件名は、静的テキスト、アラート ペイロードから抽出された動的な値
またはその両方の組み合わせでオーバーライドできます。
...
## ログ アラート V2 における件名カスタマイズ機能（プレビュー）
なお、本記事公開後のアップデートとして、ログ アラート V2 では通知メールの件名をカスタマイズできる機能が追加されています。

本機能は現在プレビュー版として提供されており、
静的な文字列やアラート ペイロードから取得した動的な値を用いて
メール件名を上書きすることが可能です。

ただし、プレビュー機能のため、将来的に仕様変更や廃止の可能性がある点にはご注意ください。

詳細な設定方法については、以下の公式ドキュメントをご参照ください。
