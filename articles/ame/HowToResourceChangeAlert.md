...
| where timestamp >= ago(5m) and timestamp <= now() // arg("") で検索するテーブルにはアラートルールの粒度や期間指定が機能しないため、クエリで期間を指定します。
```

![](./HowToResourceChangeAlert/image.png)
...
2. 例として上記サンプル クエリを指定し、[実行] をクリックすると、クエリが実行されクエリ結果が出力されます。
3. クエリ結果が出力された状態で、画面右上の [・・・] - [新しいアラート ルール] を選択することでアラートルールの作成画面へ移動します。

![](./HowToResourceChangeAlert/create_alertrule01.png)

4. 条件 セクションでは例えば以下の設定値とします。

![](./HowToResourceChangeAlert/create_alertrule02.png)
...
6. 詳細 セクションの [ID] 部分にて、 "システム割り当てマネージド ID" (System assigned managed Identity) を指定し、作成します。

ARG へのクエリにはサブスクリプションスコープの閲覧者ロールが必要ですが、当該設定にて "既定" (Default) を選択した場合、アラートルールは ARG を閲覧できず検知されません。

![](./HowToResourceChangeAlert/create_alertrule03.png)

7. アラートルールの マネージド ID の権限設定

モニター > アラート > アラート ルール > ID (プレビュー) を確認します。
この時、もしもシステム割り当て済 が オフ となっている場合にはオンにします。

![](./HowToResourceChangeAlert/create_alertrule04.png)
...
![](https://github.com/jpazmon-integ/blog/assets/20591351/2dfafd2e-4c55-4846-a03c-bb1449cd4535)
![](./HowToResourceChangeAlert/create_alertrule05.png)
